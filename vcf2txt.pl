...
our ($opt_F, $opt_R, $opt_s, $opt_r, $opt_n, $opt_N, $opt_H, $opt_q, $opt_p, $opt_b, $opt_f, $opt_c, $opt_u, $opt_D, $opt_Q, $opt_P, $opt_M, $opt_o, $opt_V, $opt_C, $opt_G, $opt_A, $opt_g, $opt_a, $opt_L, $opt_U);
getopts('suHabgLR:F:f:n:r:p:q:c:D:P:Q:M:o:V:C:G:A:N:') || USAGE();
...
my @columns = qw(GENE CDS AA CNT END DP AF BIAS PMEAN PSTD QUAL QSTD SBF CAF VD RD ALD CLNSIG ODDRATIO HIAF MQ SN ADJAF NM SHIFT3 MSI dbSNPBuildID GT DUPRATE SPLITREAD SPANPAIR);
my @ampcols = ();
my @paircols = ();
...
while( <VCF> ) {
    ...
    @paircols = qw(TYPE STATUS SSF SOR M_DP M_AF M_VD M_RD M_ALD M_BIAS M_PMEAN M_PSTD M_QUAL M_QSTD M_HIAF M_MQ M_SN M_ADJAF M_NM M_GT M_DUPRATE M_SPLITREAD M_SPANPAIR) if ( $a[10] );

    $d{ SBF } = $d{ SBF } < 0.0001 ? sprintf("%.1e", $d{ SBF }) : sprintf("%.4f", $d{ SBF }) if ( $d{ SBF } );
    $d{ ODDRATIO } = sprintf("%.3f", $d{ ODDRATIO }) if ( $d{ ODDRATIO } );
    my @effs = ();
    my @teffs = (); # up or downstream
    my @utreffs = (); # utr
    my @igeffs = (); # intergenic
    my @intraeffs = (); # intragenic
    if ( $d{ EFF } ) {
        @effs = split(/,/, $d{ EFF });
    } elsif ( $d{ ANN } && $d{ ANN } !~ /ERROR_/ ) {
        my @anns = split(/,/, $d{ ANN });
        my @fusions = ();
        foreach my $Ann (@anns) { 
            ...
            if ( $ann =~ /stream/i ) {
                    ...
                    push(@teffs, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)");
                    next;
                #}
            } elsif ( $ann =~ /_prime_UTR/ ) {
                #if ( $hgvsc && $hgvsc =~ /^[cn]\.\-(\d+)/ && $1 > $UTR ) {
                    push(@utreffs, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)");
                    next;
                #}
            } elsif ( $ann =~ /intergenic/i ) {
                push(@igeffs, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)");
                next;
            } elsif ( $ann =~ /intragenic/i ) {
                push(@intraeffs, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)");
                next;
            }

            #my @alts = split(/,/, $a[4]);
            push(@effs, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)");
            push(@fusions, "$ann($impact|$ann|$hgvsc|$hgvsp/$hgvsc|$protp|$gname|$biotype|$ftype|$fid|$rank|1)") if ( $ann =~ /fusion/i );
        }
        @effs = @fusions if ( @fusions ); # Only take fusions if present
        @effs = @utreffs unless( @effs ); # Take UTR annotation if no significant annotation of variants were found
        @effs = @teffs unless( @effs ); # Take intronic or upstream if no significant annotation of variants were found
        @effs = @intraeffs unless( @effs ); # Take intragenic if no significant annotation of variants were found
        @effs = @igeffs unless( @effs ); # Take intergenic if no significant annotation of variants were found
    } else {
        @effs = (" (||||||||||1)");
    }
    ...
    $d{ SAMPLE } = "Sample" unless( $d{ SAMPLE } );
...
sub checkCLNSIG {
    ...
    return checkCLNSIG2( $clnsig ) unless( $clnsig =~ /\d/ );
    my @cs = split(/\||,/, $clnsig );
    my $flag255 = 0;
    my $flagno = 0;
    my $flagyes = 0;
...
# This subroutine or updated Clinvar, where CLNSIG is no longer numbers
sub checkCLNSIG2 {
    my $clnsig = shift;
    return "ClnSNP_unknown" if ( $clnsig =~ /^Conflict/i );
    return "dbSNP" if ( $clnsig =~ /benign/i );
    return "ClnSNP_known" if ( $clnsig =~ /pathogenic/i );
...
print <<USAGE;
    ...
    -U INT
       Keep variants that are within INT bp (upstream or downstream) of a gene.  This will keep TERT promoter mutations as it's annotated as
       upstream.  Default: 300
